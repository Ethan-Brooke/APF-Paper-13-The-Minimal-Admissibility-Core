...
Open the interactive DAG at `https://ethan-brooke.github.io/APF-Paper-13-The-Minimal-Admissibility-Core/` (requires GitHub Pages enabled on this repo's `/docs` folder). Hover any node for its claim, epistemic tag, and shortest derivation chain. Click **Run Checks** to watch all theorems verify in topological order.
...
Expected output: one line per check, then a summary `Paper 13 (The Minimal Admissibility Core): N passed, 0 failed, N total`. Elapsed time is typically under a second.
...
Every `\coderef{check_X}{module.py}` in the paper source (`Paper_13_Minimal_Admissibility_Core_v8.3-PLEC.tex`) points to a specific check function. To verify the paper's claim at a particular point:

```bash
# find all coderefs in the paper
grep -o '\\coderef{[^}]*}{[^}]*}' Paper_13_Minimal_Admissibility_Core_v8.3-PLEC.tex
