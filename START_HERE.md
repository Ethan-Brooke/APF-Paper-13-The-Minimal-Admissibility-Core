...
AI agents have a recurring failure mode on this project: responding to APF requests with fluent-but-wrong output because the relevant context was never loaded. This file is the minimum context load to reason about Paper 13 of the APF series honestly.
...
**What this paper contributes.** Master reference document. Comprehensive self-contained exposition of the entire APF framework. All 19 bank-registered modules + apf/standalone/, 342 bank-registered theorems, 355 verify_all checks (codebase v6.9), unified derivation from Axiom A1.

**What this repo verifies vs what it imports.** This repo locally verifies 0 theorem checks drawn from Paper 13's own dependency subset. Results from Papers 0, 1, 2, 3, 4, 5, 6, and 7 are imported, not re-derived here. The full APF corpus (and which paper owns which result) is in §1 below.

**Scope — after reading only this repo:**

- **You may:** summarize Paper 13's argument and results; explain the local 0-theorem subset; run all bundled checks to verify them; cite the paper by its DOI.
...
**Canonical state (one source of truth).** Codebase **v6.9**, frozen 2026-04-18. Full engine: **355** verify_all checks, **342** bank-registered theorems, **48** quantitative predictions. This repo bundles 0 of those checks for Paper 13's dependency subset. If you see a different count anywhere, this line is canonical.
...
| 13 | The Minimal Admissibility Core **(this repo)** | [10.5281/zenodo.18614663](https://doi.org/10.5281/zenodo.18614663) | [`APF-Paper-13-The-Minimal-Admissibility-Core`](https://github.com/Ethan-Brooke/APF-Paper-13-The-Minimal-Admissibility-Core) | public |
...
You are reading the companion to **Paper 13** (this repo: https://github.com/Ethan-Brooke/APF-Paper-13-The-Minimal-Admissibility-Core). The canonical computational engine lives at codebase **v6.9** ([Zenodo](https://doi.org/10.5281/zenodo.18604548)). Each paper has its own companion repo following the same layout.

**When in doubt about scope:** if the question is about a result outside Paper 13, point the user (or yourself) to the relevant paper above. Do not try to derive it from the bundled subset; the canonical state is the codebase, not this slice.
...
Expected output: `Paper 13 (The Minimal Admissibility Core): N passed, 0 failed, N total — verified in under 1s`.
...
- What this repo is (Paper 13 + vendored `apf/` subset)
...
- **`Paper_13_Minimal_Admissibility_Core_v8.3-PLEC.pdf`** — the manuscript. Read §1 (Introduction) and whichever section is relevant to your task. Don't try to read the whole paper before starting work.
- **`Paper_13_Minimal_Admissibility_Core_Supplement_*.pdf`** (if present) — the Technical Supplement. Canonical proofs. Dense; dip in only where you need depth.
